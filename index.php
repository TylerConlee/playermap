...
<!-- Leaflet + Leaflet.markercluster, via CDN. No self-hosting needed --
     ...
     preconnect added 2026-08-20 -- lets the browser open the TLS
     connection to the CDN in parallel with parsing the rest of <head>
     instead of waiting until the first <link>/<script> tag is reached. -->
...
/* ---- Leaflet map area ----
   Replaces the old #map-wrapper / CSS-zoom / manual pixel-positioned
   <img> approach entirely. Leaflet owns its own internal pan/zoom state
   within this fixed-size container -- the container's CSS size never
   changes regardless of map zoom level. */
#leaflet-map {
    width: 100%;
    height: 78vh;
    background: #000000;
...
/* Calibration debug readout -- click anywhere on the map to see that
   point's native-image pixel coordinates (the same space wowToPixel()
   outputs into). Kept in place in case a continent ever needs another
   calibration pass -- see the note above CONTINENTS below. */
#calib-readout {
    position: fixed;
    top: 60px;
    right: 12px;
...
// Armory linking. Realm slug is matched case-insensitively by the
// Armory app's router (/character/:realm/:name) -- "hollowpeak" assumed
// to match the realm name configured in the Armory's own config.json on
// the droplet, which isn't visible from this repo (not committed,
// deployment-specific). Worth a quick check if links 404.
...
// have no real open-world x/y while inside), keyed by continent name.
var instances_x = {
...
// ============================================================
// Per-continent map configuration. wowToPixel(x, y) returns {x, y} in
// the image's own NATIVE pixel space. toLatLng() below handles the
// Y-flip Leaflet's CRS.Simple needs.
//
// EK + KALIMDOR: full 6-parameter affine transform (px = a*x + b*y + c ;
// py = d*x + e*y + f), solved 2026-08-20 from 3 ground-truth points per
// continent (Stormwind/Ironforge/Thelsamar for EK, Orgrimmar/Auberdine/
// Thunderbluff for Kalimdor). The off-diagonal coefficients in both
// solves are meaningfully non-zero, confirming the source images have
// real rotational/shear skew that a simpler independent-axis-scale model
// couldn't represent -- that mismatch was the root cause of markers
// drifting away from known landmarks the further they were from the
// original 2 calibration points. All 6 ground-truth points reproduce
// their exact target pixels under this transform. Thunderbluff's WoW
// coordinate is the one piece of ground truth NOT from a fresh .gps
// reading (reused from an earlier "Mulgore/Red Rocks" reading) -- if
// anything near Mulgore/Thunderbluff ever looks slightly off, that's the
// first point to re-verify with an actual .gps there. The #calib-readout
// click tool further down stays in the page for exactly this kind of
// future touch-up.
// ============================================================
var CONTINENTS = {
  azeroth: {
    ...
    width: 2600,
    height: 2400,
    mapIds: [0, 1], // 0 = Eastern Kingdoms, 1 = Kalimdor, share this image
  ...
  northrend: {
    ...
    width: 966,
    height: 732,
    // 609 = Ebon Hold (Death Knight starting area), plotted on the same
    // image. Given the same scale Northrend itself uses.
    mapIds: [571, 609],
    wowToPixel: function(x, y, m) {
      if (m == 609) {
        x = x - 2355; y = y + 5662;
...
// Icon objects cached once and reused across every marker, rather than
// creating a new L.icon() per marker per poll -- with up to ~2500 bots,
// that was thousands of redundant object allocations every single poll
// cycle for what are only ever 3 distinct icon images.
var ICON_ALLIANCE = L.icon({ iconUrl: IMG_BASE + "allia.gif", iconSize: [16, 16], iconAnchor: [8, 8] });
var ICON_HORDE = L.icon({ iconUrl: IMG_BASE + "horde.gif", iconSize: [16, 16], iconAnchor: [8, 8] });
var ICON_INSTANCE = L.icon({ iconUrl: IMG_BASE + "inst-icon.gif", iconSize: [20, 20], iconAnchor: [10, 10] });

function toLatLng(pixelX, pixelY, continentKey) {
  // Leaflet's L.CRS.Simple treats [0,0] as bottom-left with Y increasing
  // upward -- the opposite of normal image pixel coordinates. Flipping
  // here is the one piece of bookkeeping CRS.Simple needs.
  var h = CONTINENTS[continentKey].height;
  return [h - pixelY, pixelX];
}

// Inverse of toLatLng -- used only by the calibration debug click handler.
...
function initMap() {
  ...
  // point's native pixel coordinates. Kept for future recalibration.
...
function renderMapPoints(data)
{
  ...
  var newMarkers = []; // collected for bulk insertion below
  ...
  while (i < data.length)
  {
    ...
    if (in_array(d.map, maps_array)) {
      ...
      if (continentKey !== currentContinentKey) { i++; continue; }

      var conf = CONTINENTS[continentKey];
      var pixel = conf.wowToPixel(d.x, d.y, d.map);
      var latlng = toLatLng(pixel.x, pixel.y, continentKey);

      var marker = L.marker(latlng, { icon: isHorde ? ICON_HORDE : ICON_ALLIANCE });
      // Popup content deferred via a function instead of built eagerly --
      // with up to ~2500 markers, most popups are never opened, so this
      // avoids building/holding that HTML for markers nobody clicks.
      marker.bindPopup((function(entry) { return function() { return buildPopupHtml([entry]); }; })(entry));
  ...
  Object.keys(instanceGroups).forEach(function(key) {
    ...
    var marker = L.marker(latlng, { icon: ICON_INSTANCE });
    marker.bindPopup((function(entries) { return function() { return buildPopupHtml(entries); }; })(g.entries));
    newMarkers.push(marker);
  });

  // Bulk insertion via addLayers() (plural) -- Leaflet.markercluster's
  // dedicated bulk API, purpose-built to be far faster than calling
  // addLayer() individually in a loop for large marker counts.
  currentClusterGroup.addLayers(newMarkers);
}
...
// currentPlayerList holds the full unfiltered/unpaginated dataset (needed
// so sorting/searching/pagination all operate on the same source of
// truth); only the current page's rows (50 at a time) actually get built
// into DOM, rather than rendering all ~5000 rows on every poll.
...
{
  // Debounced 200ms -- avoids a full re-render on every keystroke while
  // typing quickly. Cheap regardless now that rendering is paginated,
  // but still an easy, free reduction in redundant work.
